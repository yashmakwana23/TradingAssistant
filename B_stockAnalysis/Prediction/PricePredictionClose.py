...
file_path = r"B_stockAnalysis\Prediction\Data\ALLHDFCBANK - Copy.csv"
...
# Check for rows with NaT (unconverted data)
unconverted_dates = df[df['Date'].isnull()]
print(unconverted_dates)
...
model.save('B_stockAnalysis\Prediction\BF_closing_prediction_model.h5')
...
def generate_future_dates(start_date, num_days):
    if pd.isnull(start_date):
        # Handle NaN or NaT for start_date
        start_date = pd.to_datetime('today')  # Set it to today's date or a default start date
